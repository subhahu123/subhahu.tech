    ...
    if ("undefined" == typeof app_domains)
        if ("undefined" == typeof app_exclude_domains && "undefined" != typeof adlinkfly_exclude_domains && (app_exclude_domains = adlinkfly_exclude_domains), "undefined" == typeof app_exclude_domains);
            ...
            for (a = 0; a < p.length; a++) {
                (t = app_get_host_name(p[a].getAttribute("href"))).length > 0 && !1 === app_domain_exist(app_exclude_domains, t) ? p[a].href = app_url + "full/?api=" + encodeURIComponent(app_api_token) + "&url=" + app_base64_encode(p[a].href) + "&type=" + encodeURIComponent(n) : "magnet:" === p[a].protocol && (p[a].href = app_url + "full/?api=" + encodeURIComponent(app_api_token) + "&url=" + app_base64_encode(p[a].href) + "&type=" + encodeURIComponent(n))
            } else
                for (var a = 0; a < p.length; a++) {
                    var t;
                    (t = app_get_host_name(p[a].getAttribute("href"))).length > 0 && app_domain_exist(app_domains, t) ? p[a].href = app_url + "full/?api=" + encodeURIComponent(app_api_token) + "&url=" + app_base64_encode(p[a].href) + "&type=" + encodeURIComponent(n) : "magnet:" === p[a].protocol && (p[a].href = app_url + "full/?api=" + encodeURIComponent(app_api_token) + "&url=" + app_base64_encode(p[a].href) + "&type=" + encodeURIComponent(n))
                }
});